...
import smtplib # Adicionado para o método síncrono direto
...
class ModernEmailSender:
    ...
    def send_email_sync_direct(self, message: EmailMessage) -> Dict[str, Any]:
        """
        Envia email de forma síncrona direta usando smtplib.
        Não usa asyncio.run, ideal para contextos síncronos onde um loop já existe.
        """
        self.logger.info(f"Iniciando envio síncrono direto para: {message.recipients}")

        try:
            # Validação prévia (reutilizando a validação existente)
            self._validate_email_message(message)
            ...
            self.logger.debug(f"Conectando a {self.config.smtp_server}:{self.config.smtp_port} (síncrono direto)...")
            with smtplib.SMTP(self.config.smtp_server, self.config.smtp_port, timeout=self.config.timeout) as server:
                # server.set_debuglevel(1) # Para debug
                if self.config.use_tls:
                    self.logger.debug("Iniciando STARTTLS (síncrono direto)...")
                    server.starttls()
                    server.ehlo() # Necessário após STARTTLS

                self.logger.debug(f"Fazendo login como {self.config.username} (síncrono direto)...")
                server.login(self.config.username, self.config.password)

                self.logger.debug("Enviando email (síncrono direto)...")
                server.sendmail(self.config.sender_email, all_recipients, mime_msg.as_string())
                self.logger.info(f"Email síncrono direto enviado com sucesso para {all_recipients}")

                return {
                    'success': True,
                    'message': 'Email enviado com sucesso (síncrono direto)',
                    'recipients_count': len(all_recipients),
                    'subject': message.subject
                }

        except smtplib.SMTPAuthenticationError as e:
            self.logger.error(f"Erro de autenticação SMTP (síncrono direto): {e}")
            raise EmailAuthenticationError("Falha na autenticação SMTP (síncrono direto)", original_error=e)
        except smtplib.SMTPConnectError as e:
            self.logger.error(f"Erro de conexão SMTP (síncrono direto): {e}")
            raise EmailConnectionError(f"Falha ao conectar ao servidor SMTP (síncrono direto): {self.config.smtp_server}:{self.config.smtp_port}", original_error=e)
        except smtplib.SMTPRecipientsRefused as e:
            self.logger.error(f"Destinatários recusados (síncrono direto): {e.recipients}")
            rejected_emails = list(e.recipients.keys())
            raise EmailRecipientError(f"Destinatários rejeitados (síncrono direto): {', '.join(rejected_emails)}", invalid_emails=rejected_emails, original_error=e)
        except EmailValidationError as e: # Re-levantar exceções de validação
            self.logger.error(f"Erro de validação (síncrono direto): {e}")
            raise
        except Exception as e:
            self.logger.error(f"Erro inesperado no envio síncrono direto: {e}", exc_info=True)
            raise EmailSendError(f"Erro inesperado no envio síncrono direto: {str(e)}", error_type="SYSTEM_ERROR_SYNC_DIRECT", original_error=e)
