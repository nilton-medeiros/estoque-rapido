...
# ToDo: Refatorar este módulo: Não permitir enviar para lixeira usuários vinculados a pedidos,
# movimento de estoque, etc, retornar False e msg_error

async def send_to_trash(page: ft.Page, cliente: Cliente) -> bool:
    operation_complete_future = asyncio.Future()
    # Definir dlg_modal ANTES de usá-lo em send_to_trash_client_async
    ...
    def send_to_trash_client_async(e_trash):
        ...
        try:
            ...
            # OPERAÇÃO SOFT DELETE: Muda o status para excluído o cliente pelo ID
            # ToDo: Verificar se há pedidos ou estoque para este cliente_id, se houver, alterar para INACTIVE
            """
            ...
            logger.info(
                f"Iniciando operação 'DELETED' para usuário ID: {cliente.id}")
            # Implemente aqui a lógica para buscar clientes, pedidos e estoque vinculados.
            # ...
            ...
            logger.info(
                f"Operação '{status.name}' para cliente ID: {cliente.id} concluída com sucesso.")
            if not operation_complete_future.done():
    ...
    text = (
        "Aviso: Este cliente será excluído permanentemente após 90 dias. "
        "Caso exista pedidos vinculados a esta cliente, "
        "ficará com o status 'Inativo' e não poderá ser excluído definitivamente."
    )
    ...
    dlg_modal = ft.AlertDialog(
        ...
        content=ft.Column(
            [
                ft.Text(cliente.name.nome_completo,
                        weight=ft.FontWeight.BOLD, selectable=True),
                ft.Text(f"ID: {cliente.id}", selectable=True),
        ...
        on_dismiss=lambda e_dismiss: (
            logger.info(
                f"Modal dialog para cliente {cliente.id} ({status.name}) foi descartado."),
...
def restore_from_trash(page: ft.Page, cliente: Cliente) -> bool:
    ...
    if result["status"] == "error":
        ...
        return False
    message_snackbar(page=page, message="Cliente restaurado com sucesso!")
    return True
