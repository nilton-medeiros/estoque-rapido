...
import src.pages.clientes.clientes_actions_page as client_actions
...
# Rota: /home/clientes/grid/lixeira
def show_clients_grid_trash(page: ft.Page):
    """Página de exibição em formato Cards dos clientes da empresa logada que estão 'DELETED'"""
    ...
    async def handle_action_click(e):
        ...
        cliente = e.control.data.get('data')

        match action:
            case "RESTORE":
                is_restore = client_actions.restore_from_trash(page=page, cliente=cliente)
                if is_restore:
                    # Reexecuta o carregamento. Atualizar a lista de clientes na tela
    ...
    appbar = ft.AppBar(
        leading=ft.Container(
            ...
            content=ft.Container(
                ...
                content=ft.Icon(
                    ft.Icons.ARROW_BACK),
                on_click=lambda _: page.go("/home/clientes/grid"), tooltip="Voltar",
                clip_behavior=ft.ClipBehavior.ANTI_ALIAS
            ),
        ),
        title=ft.Text(f"Clientes excluídos", size=18),
    ...
    def handle_info_click(e):
        ...
        info_title = "Informação do Cliente"
        info_message = ""

        if cliente.status == RegistrationStatus.DELETED:
            info_message = recycle_helpers.get_deleted_info_message(cliente)
        else:
        ...
        info_dialog = ft.AlertDialog(
            ...
            on_dismiss=lambda _: logger.info(f"Dialog de informação para {cliente.id} dispensado.")
        )
    ...
    async def load_data_and_update_ui():
        ...
        try:
            ...
            if not empresa_id:  # Só busca as clientes da empresa logada, se houver ID
                content_area.controls.append(empty_content_display)
                return

            result = client_controllers.handle_get_all(empresa_logada=empresa_id, status_deleted=True)

            if result["status"] == "error":
                content_area.controls.append(empty_content_display)
                return

            clientes_data = result['data']["clientes"]
            clientes_deleted_count: int = result['data']["quantidade_deletados"]
                ...
                controls=[
                    create_recycle_bin_card(
                        ...
                        date_text_control=ft.Text(
                            value=f"Excluído em: {format_datetime_to_utc_minus_3(cliente.deleted_at)}",
                            color=ft.Colors.RED,
            ...
            logger.error(f"Ocorreu um erro ao carregar os clientes deletados. Tipo: {type(e).__name__}, Erro: {e}\nTraceback:\n{tb_str}")
            ...
            content_area.controls.append(
                ft.Container(
                    content=ft.Column([
                        ...
                        ft.Text(
                            f"Ocorreu um erro ao carregar os clientes deletados.", color=ft.Colors.RED),
            ...
            current_trash_icon_filename = "recycle_full_1771.png" if clientes_deleted_count else "recycle_empy_1771.png"
    ...
    return ft.View(
        route="/home/clientes/grid/lixeira",  # A rota que esta view corresponde
        controls=[
