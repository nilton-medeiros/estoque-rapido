...
def show_products_grid_trash(page: ft.Page):
    ...
    def handle_info_click(e):
        ...
        if produto.status.name == 'DELETED':
            if produto.deleted_at:  # Verifica se a data de exclusão está definida
                # Data em que o item foi movido para a lixeira (presumivelmente UTC)
                data_movido_lixeira = produto.deleted_at
                ...
                info_message = "Este produto está na lixeira, mas a data de início da contagem para exclusão não foi registrada."
        else:
            ...
            grid = ft.ResponsiveRow(
                controls=[
                    ...
                    # Por isso, não é possível movê-lo para uma variável para reduzir o nível de aninhamento.
                        ...
                        content=ft.Container(
                            ...
                            content=ft.Column([
                                ft.Row(
                                    controls=[
                                        ft.Container(
                                            image=ft.DecorationImage(src=produto.image_url),
                                            width=100,
                                            height=100,
                                            border_radius=ft.border_radius.all(10),
                                        ...
                                        ft.Container(
                                            ...
                                            content=ft.PopupMenuButton(
                                                ...
                                                items=[
                                                    ft.PopupMenuItem(
                                                        text="Restaurar",
                                                        tooltip="Restaurar produto da lixeira",
                                                        icon=ft.Icons.RESTORE,
                                                        data={
                                                            'action': 'RESTORE', 'data': produto},
                                ...
                                ft.Text(
                                    f"{produto.name}", color=ft.Colors.WHITE70, weight=ft.FontWeight.BOLD),
                                ft.Text(f"{produto.description if produto.description else ''}",
                        ...
                        tooltip=f"{'Exclusão automática e permanente após 90 dias na lixeira' if produto.status.name == 'DELETED' else 'produto arquivada não será removida do banco de dados! Pode estar vinculada a pedidos ou estoque.'}",
                    ) for produto in produtos_data  # Criar um card para cada empresa
                ],
